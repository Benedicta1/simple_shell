#include "shell.h"

/**
 * our_strlen - This returns the length of a string
 * @s: This is the string whose length to check
 *
 * Return: integer length of string
 */
int our_strlen(char *s)
{
	int a = 0;

	if (!s)
		return (0);

	while (*s++)
		a++;
	return (a);
}

/**
 * our_strcmp - This performs lexicogarphic comparison of two strangs.
 * @s1: the first strang
 * @s2: the second strang
 *
 * Return: negative if s1 < s2, positive if s1 > s2, zero if s1 == s2
 */
int our_strcmp(char *s1, char *s2)
{
	while (*s1 && *s2)
	{
		if (*s1 != *s2)
			return (*s1 - *s2);
		s1++;
		s2++;
	}
	if (*s1 == *s2)
		return (0);
	else
		return (*s1 < *s2 ? -1 : 1);
}

/**
 * our_starts_with - This checks if needle starts with haystack
 * @haystack: This is the string to search
 * @needle: This is the substring to find
 *
 * Return: address of next char of haystack or NULL
 */
char *our_starts_with(const char *haystack, const char *needle)
{
	while (*needle)
		if (*needle++ != *haystack++)
			return (NULL);
	return ((char *)haystack);
}

/**
 * our_strcat - This concatenates two strings
 * @dest: This is the destination buffer
 * @src: This is the source buffer
 *
 * Return: pointer to destination buffer
 */
char *our_strcat(char *dest, char *src)
{
	char *ret = dest;

	while (*dest)
		dest++;
	while (*src)
		*dest++ = *src++;
	*dest = *src;
	return (ret);
}

